...
const mockNutritionData = [
  { name: "Breakfast", value: 8, color: "#ff6b6b", icon: "🌅" },
  { name: "Lunch", value: 6, color: "#4ecdc4", icon: "☀️" },
  { name: "Dinner", value: 7, color: "#45b7d1", icon: "🌙" },
  { name: "Snacks", value: 12, color: "#f9ca24", icon: "🍎" }
];
  ...
  useEffect(() => {
    const fetchNutritionLogs = async () => {
      try {
        ...
        const res = await axios.get("http://localhost:5000/nutrition", {
          headers: { 
        ...
        const weekCount = weeklyLogs.length;
        setWeeklyCount(weekCount);

        // Process data for pie chart (you can customize this based on your data structure)
        // This example categorizes by meal type - adjust based on your API response
        const processedData = mockNutritionData.map(category => ({
          ...category,
          value: Math.floor(Math.random() * 10) + 5 // Replace with actual categorization logic
        }));

        // Fallback to mock data if no real data available
        const finalData = logs.length > 0 ? processedData : mockNutritionData;
        
        setNutritionData(finalData);
        setTotalLogs(finalData.reduce((sum, item) => sum + item.value, 0));
        ...
        setTotalLogs(mockNutritionData.reduce((sum, item) => sum + item.value, 0));
        setWeeklyCount(mockNutritionData.reduce((sum, item) => sum + item.value, 0));
      } finally {
        setLoading(false);
      }
    };
                    ...
                    >
                      {nutritionData.map((entry, index) => (
                        <Cell 
                          ...
                          style={{
                            filter: index === activeIndex ? "brightness(1.1) drop-shadow(0 0 10px rgba(255,255,255,0.3))" : "none",
